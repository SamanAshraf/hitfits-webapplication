...
<nav class="navbar navbar-light bg-light bg-white py-0"  id="nav">
  ...
  <div class="container-fluid cart-page">
      ...
      <div class="empty">
        <p>Shopping Cart is empty. Add products to proceed.</p>
        <a href="{{url('/index')}}"> <button type="submit" class="btnH">Home</button></a>
      </div>
      ...
      <table id="MyTable">
        ...
        @if(session('cart'))
            @foreach(session('cart') as $id => $details)
                @php $total += $details['salesprice'] * $details['quantity'] @endphp
            <tr data-id="{{ $id }}">
              <td>
                <div class="cart-info">
                  <img src="{{\Illuminate\Support\Facades\Storage::url($details['img'])}}">
                  <div>
                    <p>{{ $details['productname'] }}</p>
                    <small>RS {{ $details['salesprice'] }}</small>
              ...
              <td >
                <input type="number" value="{{ $details['quantity'] }}" class="quantity update-cart" id="quantity">
              </td>
              <td class="price">RS {{ $details['salesprice'] }}</td>
            
              <td><a href="" class="Btn remove-from-cart"><b>Remove</b></a></td>
            </tr>
    ...
    <div class="total-price">
      ...
      <table id="totalp">
        ...
        <tr>
          ...
          <td class="delivery">RS 200</td>
        </tr>
        <tr>
          <td>Total</td>
          <td class="total">RS {{ $total + 200}}</td>
        </tr>
      </table>
      
    </div>
   
    <a href="deliveryform"> <button type="submit" class="btn btn-light btn-block chk">Checkout</button></a>
    @endif
  </div>
  ...
  <footer class="site-footer">
    ...
    <script async>
     ...
     /*   // remove item 
      var removebtn = document.getElementsByClassName("Btn")
      ...
      function updateT(){
        ...
        var qty = row.cells.item(3).childNodes[1].value
    
        var priceE = row.cells.item(4)
      ...
      if(total==0){
        ...
        //document.querySelector(".delivery").innerText = `Rs 0`

        document.querySelector(".flows").style.display = `none`
        document.querySelector(".total-price").style.display = `none`
        document.querySelector(".chk").style.display = `none`
        document.querySelector(".empty").style.display = `block`
          
      }else{
        document.querySelector(".total").innerText = `Rs ${total+200}`
      }
      }  

        
        //var deliveryElement = document.getElementsByClassName("delivery")[0]

        //var deliveryprice = parseFloat(deliveryElement.innerText.replace('RS',''))
       // alert(deliveryprice)
          

      /*  if(total!=0){
          document.getElementsByClassName("total")[0].innerText = 'RS' + total + deliveryprice
        }else{
          document.getElementsByClassName("delivery")[0].innerText = 'RS' + 0
          document.getElementsByClassName("total")[0].innerText = 'RS' + total

          //document.getElementsByClassName("flows").style.display = "none";
          //document.getElementsByClassName("cart-page").style.display = "none";
